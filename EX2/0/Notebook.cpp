...
void Notebook::write(int page, int row, int column, Direction direction, const string &toWrite)
{
    ...
    if ((int)toWrite.length() > __maximum_line_length||column + (int)toWrite.length() > __maximum_line_length)
    {
    ...
    {
       if (ch<minInvalidValue||ch>maxInvalidValue||ch=='_'||ch=='~'){
      throw runtime_error("Invalid input.\n Do not enter an invalid character.");
    }

    }
    this->startLine(page,row,direction,toWrite.length());
    if(row !=0)
    {
           this->startLine(page,row-1,direction,toWrite.length());

    }
     this->startLine(page,row+1,direction,toWrite.length());
    if(!checkWrite(page,row,column,direction,toWrite.length()))
    {
...
string Notebook::read(int page, int row, int column, Direction direction, int lenChar)
{
    ...
    if ( lenChar  > __maximum_line_length||column + lenChar  > __maximum_line_length)
    {
         throw runtime_error("Invalid input.\n Do not enter more than 100 characters");
    }
