 ...
 * @return New matrix
 */
	Matrix::Matrix(const std::vector<double>& matrix,int row,int column) noexcept(false) 
	{
		if (row<0||column<0)
		{
 ...
 */
	Matrix Matrix::operator*(const Matrix& other) const noexcept(false)
	{
		...
		size_t size=static_cast<uint>(this->_row*other._column);//Size of the new matrix
